        ...
        <div className="flex flex-col gap-2 lg:gap-3">
            {[0, 1, 2, 3].map(y => (
                <div key={y} className="flex flex-row gap-2 lg:gap-3">
                    {[0, 1, 2, 3].map(x => (
                        <div key={x} onClick={() => clicked([x, y, z])} className="w-11 h-11 sm:w-14 sm:h-14 md:w-16 md:h-16 lg:w-20 lg:h-20 border-2 rounded sm:rounded-md md:rounded-xl bg-stone-300 flex">
                            { sub[y]![x] != 0 &&
